...
if __name__ == "__main__":
    orderNumber = '8'
    data_dir = "distributed_complete_dataset"
    Identifier = "270LSTMDistributed_for_order_{}".format(orderNumber)
    model_collection = glob.glob('{}models/{}_choose_3_*.h5'.format(parent_dir, Identifier))
    dataset_path = glob.glob("{}{}/*.csv".format(parent_dir, data_dir))
    # set up proper columns based on orders
    current_order_dict = orders_lookuptable.get(str(orderNumber))
